...
window.onload = function init()
{
    ...
    makeSphere(0.5, 15, 15);
    // lets make a core as well;
    makeSphere(0.2,5,5);
	
	//HW470: Create colors for the core and outer parts
    // See HW specs for the number of colors needed

	vertices.forEach( vert => colors.push(vec3(Math.abs(vert[1])+0.1,Math.abs(vert[2])/2, Math.abs(vert[1]) ) ));
...
function render()
{
    ...
    let viewMatrix = lookAt(vec3(0,0,-2),vec3(0,0,1), vec3(0,1,0));
    gl.uniformMatrix4fv(viewLocation, false, flatten(viewMatrix));
...
function makeSphere (radius, rings, sectors) {
    ...
    for(r = 0; r < rings-1; r++) for(s = 0; s < sectors; s++) {
        tempInd.push( r * sectors + s );
        tempInd.push( r * sectors + (s+1) );
        tempInd.push( (r+1) * sectors + (s+1) );
        tempInd.push( (r+1) * sectors + s );
        tempInd.push( r * sectors + (s+1) );
        tempInd.push( (r+1) * sectors + (s+1) );
    }

    indicies = indicies.concat(tempInd.filter(i => i < vertices.length).map(i => i+curVert));
}
